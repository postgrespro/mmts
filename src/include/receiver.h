...
typedef enum
{
	REPLMODE_RECOVERY,			/* perform recovery of the node by applying
								 * all data from the slot from specified point */
	REPLMODE_RECOVERED			/* recovery of receiver node is completed so
								 * drop old slot and restart replication from
								 * the current position in WAL */
} MtmReplicationMode;
